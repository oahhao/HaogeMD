...
打开文件后，阅读区占满整个窗口（100vh × 100vw），所有 UI 元素浮动在内容上方：

```
┌──────────────────────────────────────────────────────┐
│  ErgeMD  │ tab1 │ tab2 │ tab3 │         [—][□][×]   │  ← TitleBar (position: fixed, top: 0)
│                                                      │    阅读时自动渐隐，鼠标移到顶部显示
├──────────────────────────────────────────────────────┤
│                                                      │
│  ┌──────┐                                           │
│  │ TOC  │          正文内容...                        │  ← ReadingArea (100vh 全屏滚动)
│  │(浮动)│          （全屏，无 Grid/Flex 嵌套）         │
│  │      │                                           │
│  │·H1 ✓ │                                           │
│  │·H2   │                                           │
│  │·H2   │                                           │
...
**布局要点：**
- ReadingArea 直接 `width: 100%; height: 100vh`，是唯一的滚动容器
- TitleBar / StatusBar 使用 `position: fixed`，浮动在内容上方
- FloatingTOC / LeftPanel / RightPanel 均为 `position: fixed`
- 不使用 CSS Grid 或 Flexbox 嵌套布局（避免滚动容器被包裹）
